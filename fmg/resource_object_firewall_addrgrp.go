...
func resourceObjectFirewallAddrgrpCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		...
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallAddrgrp(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallAddrgrp(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallAddrgrpUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallAddrgrp(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallAddrgrp resource: %v", err)
	}
...
func resourceObjectFirewallAddrgrpDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallAddrgrp(mkey, paradict, wsParams)
	if err != nil {
...
func resourceObjectFirewallAddrgrpRead(d *schema.ResourceData, m interface{}) error {
	...
	cached, err := globalBulkCache.GetOrLoad(cacheKey, func() ([]interface{}, error) {
		return c.BulkGetObjectFirewallAddrgrp(adomv)
	})
	if err != nil {
