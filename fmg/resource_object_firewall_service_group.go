...
func resourceObjectFirewallServiceGroupCreate(d *schema.ResourceData, m interface{}) error {
	...
	if update_if_exist && mkey_ok {
		// check existing
		o, err = c.ReadObjectFirewallServiceGroup(mkey, paradict)
		if err == nil && o != nil {
			existing = true
			// update if existing
			o, err = c.UpdateObjectFirewallServiceGroup(obj, mkey, paradict, wsParams)
			if err != nil {
	...
	if !existing {
		_, err = c.CreateObjectFirewallServiceGroup(obj, paradict, wsParams)
		if err != nil {
...
func resourceObjectFirewallServiceGroupUpdate(d *schema.ResourceData, m interface{}) error {
	...
	_, err = c.UpdateObjectFirewallServiceGroup(obj, mkey, paradict, wsParams)
	if err != nil {
		return fmt.Errorf("Error updating ObjectFirewallServiceGroup resource: %v", err)
	}
...
func resourceObjectFirewallServiceGroupDelete(d *schema.ResourceData, m interface{}) error {
	...
	err = c.DeleteObjectFirewallServiceGroup(mkey, paradict, wsParams)
	if err != nil {
...
func resourceObjectFirewallServiceGroupRead(d *schema.ResourceData, m interface{}) error {
	...
	cacheKey := "/pm/config/" + adomv + "/obj/firewall/service/group"
	cached, err := globalBulkCache.GetOrLoad(cacheKey, func() ([]interface{}, error) {
		return c.BulkGetObjectFirewallServiceGroup(adomv)
	})
	if err != nil {
	...
	err = refreshObjectObjectFirewallServiceGroup(d, oMap)
	if err != nil {
